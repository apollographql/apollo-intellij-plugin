...
class ApolloFieldInsightsInspection : LocalInspectionTool() {
  ...
  private fun getLatency(field: GraphQLField): Double? {
    val fieldDefinition =
      field.findChildrenOfType<GraphQLIdentifier>().firstOrNull()?.reference?.resolve()?.parent as? GraphQLFieldDefinition
          ?: return null
  ...
  override fun createOptionsPanel() =
    SingleIntegerFieldOptionsPanel(ApolloBundle.message("inspection.fieldInsights.settings.threshold"), this, "thresholdMs")
  ...
  private class EncloseInDeferredFragmentQuickFix : LocalQuickFix {
    ...
    override fun applyFix(project: Project, descriptor: ProblemDescriptor) {
      ...
      val newSelection =
        (graphQLFile.firstChild as GraphQLSelectionSetOperationDefinition).selectionSet.selectionList[0] as GraphQLSelection
